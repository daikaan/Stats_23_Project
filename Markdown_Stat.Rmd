...
| Credit_Limit | Avg_Open_To_Buy | 0.92 |
| Credit_Limit | Avg_Utilization_Ratio | 0.63 |
| Total_Revolving_Bal | Avg_Open_To_Buy | 0.26 |
| Total_Revolving_Bal | Avg_Utilization_Ratio | 0.64 |
| Avg_Open_To_Buy | Avg_Utilization_Ratio | -0.85 |
| Total_Amt_Chng_Q4_Q1 | Total_Ct_Chng_Q4_Q1 | 0.38 |
| Total_Trans_Amt | Total_Trans_Ct | 0.90 |
...
b <- ggplot(train, aes(x = Total_Trans_Amt, fill = Customer)) +
  geom_density(alpha=0.3) + ggtitle("Total_Trans_Amt") +
  scale_fill_manual(values = c("darkgrey","red"),labels = c("Existing","Attrited"))

c <- ggplot(train, aes(x = Total_Ct_Chng_Q4_Q1, fill = Customer)) +
...
h <- ggplot(train, aes(x = Avg_Open_To_Buy, fill = Customer)) +
  geom_density(alpha=0.3) + ggtitle("Avg_Open_To_Buy") +
  scale_fill_manual(values = c("darkgrey","red"),labels = c("Existing","Attrited"))
...
For *Avg_Open_To_Buy* we notice that there is a difference between the classes of Attriting and Existing Customer but it's not enough to infer a clear relationship between Attriting customers and the variable analyzed.
...
To mitigate this effect we remove the indipendent variable *Avg_Open_To_Buy*. We also remove *Months_on_book* since, from the summary, it seems that it is not significant and it would be removed with the stepwise selection.

```{r}
glm_2 <- update(glm_1, . ~ . - Avg_Open_To_Buy - Months_on_book)
...
```{r}
glm_3 <- update(glm_2, . ~ . + Total_Amt_Chng_Q4_Q1*Customer_Age 
                + Total_Amt_Chng_Q4_Q1*Dependent_count 
                + Total_Amt_Chng_Q4_Q1*Total_Trans_Amt 
                + Total_Amt_Chng_Q4_Q1*Total_Trans_Ct)

glm_4 <- update(glm_3, . ~ . + Total_Revolving_Bal*Credit_Limit 
                + Total_Revolving_Bal*Avg_Utilization_Ratio)

glm_5 <- update(glm_4, . ~ . + Credit_Limit*Avg_Utilization_Ratio)

glm_6 <- update(glm_5, . ~ . + Total_Ct_Chng_Q4_Q1*Is_Female)
...
We decided to not remove *Total_Amt_Chng_Q4_Q1:Total_Trans_Ct* since removing it actually increases the *AIC*.
Instead, we decided to keep *Total_Trans_Amt* since it's been used in a interaction.

We now check the AIC values.
We can notice that it gradually gets smaller except in the step where we removed *Avg_Open_To_Buy* to reduce the collinearity.
```{r}
...
```{r ,fig.align='center',fig.asp= 0.9, fig.width= 7, warning=FALSE}
i1 <- interact_plot(glm_3,pred = Total_Amt_Chng_Q4_Q1,modx = Customer_Age,
                    outcome.scale = "link")
i2 <- interact_plot(glm_3,pred = Total_Amt_Chng_Q4_Q1,modx = Dependent_count,
                    outcome.scale = "link")
i3 <- interact_plot(glm_3,pred = Total_Amt_Chng_Q4_Q1,modx = Total_Trans_Amt,
                    outcome.scale = "link")
i4 <- interact_plot(glm_3,pred = Total_Amt_Chng_Q4_Q1,modx = Total_Trans_Ct,
                    outcome.scale = "link")
i5 <- interact_plot(glm_4,pred = Total_Revolving_Bal,modx = Credit_Limit,
                    outcome.scale = "link")
i6 <- interact_plot(glm_4,pred = Total_Revolving_Bal,modx = Avg_Utilization_Ratio,
                    outcome.scale = "link")
i7 <- interact_plot(glm_5,pred = Credit_Limit,modx = Avg_Utilization_Ratio,
                    outcome.scale = "link")
i8 <- interact_plot(glm_6,pred = Total_Ct_Chng_Q4_Q1,modx = Is_Female,
                    outcome.scale = "link")
...
We check for collinearity and, as for the unbalanced case, we decide to remove the variable *Avg_Open_To_Buy* and *Months_on_book*.
...
```{r}
glm_2_bal <- update(glm_1_bal, . ~ . - Avg_Open_To_Buy - Months_on_book)
...
```{r}
glm_3_bal <- update(glm_2_bal, . ~ . + Total_Amt_Chng_Q4_Q1*Customer_Age +
                      Total_Amt_Chng_Q4_Q1*Dependent_count +
                      Total_Amt_Chng_Q4_Q1*Total_Trans_Amt)
glm_4_bal <- update(glm_3_bal, . ~ . + Total_Revolving_Bal*Credit_Limit +
                      Total_Revolving_Bal*Avg_Utilization_Ratio)
glm_5_bal <- update(glm_4_bal, . ~ . + Credit_Limit*Avg_Utilization_Ratio)
glm_6_bal <- update(glm_5_bal, . ~ . + Total_Ct_Chng_Q4_Q1*Is_Female)
...
We can notice that the AUC value for the Balanced LDA is even higher than the best model obtained by GLM, even if the F1 score is lower.
...
```{r}
# Matrix without Attrition_Flag, Months_on_book and Avg_Open_To_Buy
...
We plot the ROC curves and corresponding AUC values of the two models.
We notice that the ROC curves and the AUC values are nearly equal, however the balanced model had a higher F1 score and is then a better choice for our problem.

```{r ,fig.align='center',fig.asp= 0.75, fig.width= 7, warning=FALSE, message=FALSE}
#ROC curves
