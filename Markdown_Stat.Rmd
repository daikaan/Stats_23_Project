...
It can be seen that the dataset is unbalanced. We decide to create a balanced training set to represent better the Attrited Customer. To define this new Training set we utilized a mix of Oversampling and Undersampling to obtain a dataset with the same length of the original.
...
In the last figure, we analyzed the correlations between the variables in such a way that takes into account the relantionships between the two variables and the other variables present in the training set.
...
#Partial correlation (We will not show the output, since it takes to much space)
...
We can also notice that many variables that had significant correlations have lower value when analyizing the  partial correlations.
...
We apply Ridge logistic regression to our matrix and we plot the misclassification error, which depends on the values of log($\lambda$), where $\lambda$ is the parameter that controls the amount of penalization.
We can notice that the increase of the penalization leads to an increase of the misclassification error
