...
knitr::opts_chunk$set(fig.width=6, fig.height=4) #setting figure sizes
...
For the 6 categorical columns, we change their data form to numeric to be able to implement a model. However, after this point the use of these columns should be done with extra care to not confuse the categorical variables with the numeric ones.
...
It is clear that many of the numerical columns have too many outliers. However, as a result of the nature of our study, eliminating the outliers might significantly affect the soundness of study. For example, credit limit column only has a lot of outliers on the higher end and eliminating them makes little sense on both theoretical and practical sense. On the theoretical side, by eliminating so, we will be losing so many people while we still do not know if the credit limit is going to be a significant factor in our model. On the practical sense, it is easy to come to the consensus of if a person is having a high credit score they will be less willing change their service provider. Another example is contacts count in the last 12 months. While the theoretical aspect stays the same, as the contacts increase the customer satisfaction is expected to decrease, since most of the contacts are expected to be the sign of a problem that requires solution. Therefore, we believe that eliminating the outliers will decrease the quality of the model significantly on the all columns but the customer age.
